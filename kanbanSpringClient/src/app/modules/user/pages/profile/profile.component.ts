...
export class ProfileComponent implements OnInit {
  ...
  UserProfileDetailResponse: UserProfileDetailResponse = {} as UserProfileDetailResponse;
  ...
  ngOnInit(): void {
    ...
    this.uploader.onWhenAddingFileFailed = (item, filter) => {
      ...
      switch (filter.name) {
        case 'fileSize':
          message = 'The file"' + item.name + '\" is ' + this.formatBytes(item.size) +
            ', this exceeds the maximum allowed size of ' + this.formatBytes(maxFileSize5MB);
          break;
        default:
  ...
  changeEmail() {
    this.changeEmailDialogService.confirm(this.UserProfileDetailResponse)
  ...
  changePassword() {
    this.changePasswordDialogService.confirm(this.UserProfileDetailResponse.id)
      .then(async apiResponse => {
        if (apiResponse) {
          this.notificationService.open(apiResponse.message);
        }
  ...
  updateName() {
    this.formNameDialogService.confirmed(this.UserProfileDetailResponse.name).then(
      async result => {
        if (result) {
          this.userService.updateName(this.UserProfileDetailResponse.id, result)
  ...
  setProfilePicture(profilePicture: any) {
    this.userService.setProfilePicture(this.UserProfileDetailResponse.id, profilePicture)
      .subscribe(apiResponse => {
        this.getCurrentUser();
        this.notificationService.open(apiResponse.message);
      })
