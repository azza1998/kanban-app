...
import { CardDetailsResponse, DescriptionRequest } from '../../interface/board-details.interface';
...
export class CardDetailsComponent implements OnInit {
  ...
  uploadAttachments(filesList: any) {
    let files = filesList;
    let formData = new FormData();
    files.forEach((file: any) => {
      formData.append('file[]', file._file);
    });
    this.cardService.uploadFiles(this.data.id, formData)
  ...
  findCardById() {
    this.cardService.findById(this.data.id)
      .subscribe(card => {
        this.card = card;
