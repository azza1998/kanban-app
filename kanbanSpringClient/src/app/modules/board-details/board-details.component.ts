...
export class BoardDetailsComponent implements OnInit {
  ...
  findBoardsById() {
    this.boardService.findById(this.id)
      .subscribe(board => {
        this.board = board;
  ...
  saveCard(idColumn: string) {
    this.formNameDialogService.confirmed()
      .then(async result => {
        if (result) {
          this.cardService.saveName(idColumn, result)
  ...
  updateCardName(id: string, name: string) {
    this.formNameDialogService.confirmed(name)
      .then(async result => {
        if (result) {
  ...
  onDropColumns(event: CdkDragDrop<any>) {
    ...
    if (currentIndex !== desiredIndex) {
      this.columnService.changePosition(idBoardReference, currentIndex, desiredIndex).subscribe(
        apiResponse => {
          this.notificationService.open(apiResponse.message);
          this.findBoardsById();
        }, error => {
  ...
  onDropCards(event: CdkDragDrop<any>) {
    ...
    if (desiredData === undefined || currentData.idColumn !== desiredData.idColumn) {
      this.cardService.moveCardToAnotherColumn(currentData.id, desiredIndex, event.container.id).subscribe(
        apiResponse => {
          this.notificationService.open(apiResponse.message);
          this.findBoardsById();
        }, error => {
      ...
      this.cardService.changeCardPosition(idColumn, currentIndex, desiredIndex).subscribe(
        apiResponse => {
          this.notificationService.open(apiResponse.message);
          this.findBoardsById();
        }, error => {
